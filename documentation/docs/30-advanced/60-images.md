...
Vous pouvez aussi importer manuellement un fichier d'image et le passer à une balise `<enhanced:img>`. Ceci est utile lorsque vous avez une collection d'images statiques et souhaitez en choisir une dynamiquement, ou bien si vous souhaitez [itérer sur les images](https://github.com/sveltejs/kit/blob/master/sites/kit.svelte.dev/src/routes/home/Showcase.svelte). Dans ce cas, vous avez besoin de mettre à jour à la fois la déclaration `import` et l'élément `<img>` comme montré ci-dessous pour indiquer que vous souhaitez les traiter.
...
- Envisagez de servir toutes les images via un <span class='vo'>[CDN](PUBLIC_SVELTE_SITE_URL/doc/web/cdn)</span> quelque soit le type d'optimisation que vous choisissez. Les CDNs permettent de réduire la latence en distribuant globalement des copies de vos fichiers statiques.
