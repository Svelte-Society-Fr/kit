...
The `$env/static/private` and `$env/dynamic/private` modules, which are covered in the [modules](modules) section, can only be imported into modules that only run on the server, such as [`hooks.server.js`](hooks#hooks-de-serveur) or [`+page.server.js`](routing#page-page-server-js).
