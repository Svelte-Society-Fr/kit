...
Si vous fournissez vos propres <span class="vo">[callbacks](PUBLIC_SVELTE_SITE_URL/docs/development#callback)</span>, vous pourriez avoir besoin de reproduire en partie le comportement par défaut de `use:enhance`, comme par exemple afficher le composant `+error` le plus proche en cas d'erreur. La plupart du temps, appeler l'argument `update` du callback suffit. Si vous avez besoin de plus de personnalisation, vous pouvez utiliser `applyAction` :
...
- `redirect` — appelle `goto(result.location)`
